...
export const getDocument = async (req, res) => {
    try {
        ...
        const isCollaborator = doc.collaborators.some((c) => c.user._id.toString() === userId);
...
export const shareDocument = async (req, res) => {
    try {
        ...
        const existingCollab = doc.collaborators.find((c) => c.user.toString() === userToShare._id.toString());
        if (existingCollab) {
...
export const joinDocument = async (req, res) => {
    try {
        ...
        const existing = doc.collaborators.find((c) => c.user.toString() === userId.toString());
        if (existing) {
            const populated = await doc.populate([
                { path: 'owner', select: 'name email avatar' },
                { path: 'collaborators.user', select: 'name email avatar' },
...
export const toggleStar = async (req, res) => {
    try {
        ...
        const isStarred = doc.isStarredBy.some((id) => id.toString() === userId.toString());
        if (isStarred) {
            doc.isStarredBy = doc.isStarredBy.filter((id) => id.toString() !== userId.toString());
        }
        else {
...
export const restoreVersion = async (req, res) => {
    try {
        ...
        const version = doc.versions.find((v) => v._id.toString() === req.params.versionId);
        if (!version) {
...
export const getYjsState = async (req, res) => {
    try {
        const doc = await DocumentModel.findOne({
            _id: req.params.id,
            isDeleted: false,
...
export const saveYjsState = async (req, res) => {
    try {
